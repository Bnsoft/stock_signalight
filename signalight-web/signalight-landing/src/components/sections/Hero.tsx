        ...
        <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-16">
          ...
          <div className="flex-1 text-center lg:text-left">
            <AnimateIn from="bottom" delay={0}>
              <Badge variant="outline" className="mb-5 text-signal-green border-signal-green/30 bg-signal-green/5">
                🚀 Now in Beta · Free to use
              </Badge>
            </AnimateIn>

            <AnimateIn from="bottom" delay={80}>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight leading-[1.15] mb-5">
                Signal + Light.{" "}
                <span className="text-signal-green">When the signal fires,</span>{" "}
                you see the light.
              </h1>
            </AnimateIn>
            ...
            <AnimateIn from="bottom" delay={240}>
              <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
                <Button size="lg" className="bg-signal-green hover:bg-signal-green/90 text-black font-semibold px-8 h-12">
                  Start Free
                </Button>
                <Button size="lg" variant="outline" className="gap-2 h-12">
          ...
          <AnimateIn from="right" delay={200} className="flex-1 w-full max-w-md lg:max-w-none">
            <div className="rounded-xl border border-border bg-card shadow-xl overflow-hidden glow-green">
              ...
              <div className="p-4 space-y-3 font-mono text-sm">
                {MOCK_SIGNALS.map((s, i) => (
                  <div key={i} className={`rounded-lg border px-4 py-3 ${SEVERITY_COLORS[s.type]}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-semibold">{s.emoji} {s.type} · {s.symbol}</span>
