    ...
    <section id="how-it-works" className="py-16 sm:py-20 px-4 sm:px-6 bg-muted/20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-6 relative">
          <div className="hidden md:block absolute top-11 left-[calc(33.33%+1.5rem)] right-[calc(33.33%+1.5rem)] h-px bg-border" />

          {STEPS.map((s, i) => {
            const Icon = s.icon
            return (
              <AnimateIn key={s.step} from="bottom" delay={i * 100}>
          ...
          <div className="mt-10 sm:mt-12 max-w-xl mx-auto">
            <div className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className="p-4 font-mono text-sm space-y-1 overflow-x-auto">
                <p><span className="text-muted-foreground select-none">$ </span><span className="text-signal-green">cp</span> .env.example .env</p>
                <p><span className="text-muted-foreground select-none">$ </span><span className="text-signal-green">uv</span> run python -m src.app</p>
