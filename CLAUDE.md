...
- **DB**: SQLite via `store.py`, file at `signalight-engine/data/`
- **Auth**: Custom JWT
- **API Base**: `http://localhost:8000` | Frontend: `http://localhost:3000`

## Repo Layout
```
signalight-engine/src/   → Core Python modules (see docs/backend/modules.md)
signalight-web/signalight-landing/src/app/dashboard/   → 6 active pages
docs/                    → Reference docs
```
...
- `/dashboard/profile` — Profile + Telegram settings
...
| CSV export        | `docs/features/export.md`        |
...
- User-facing strings (UI labels) may remain in Korean
